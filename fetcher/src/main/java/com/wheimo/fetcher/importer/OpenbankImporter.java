...
@Slf4j
public class OpenbankImporter implements BankImporter {
    ...
    public SyncResultMessage fetchTransactions(SyncRequestMessage request, String decryptedPassword) {
        try {
            ...
            String contract = request.getSettings() != null ? request.getSettings().get("contract") : null;
            String product = request.getSettings() != null ? request.getSettings().get("product") : null;
            List<SyncResultMessage.ImportedTransaction> transactions = getTransactions(token, contract, product, request.getFrom(), request.getAccountId());
            ...
            log.error("Openbank import failed", e);
    ...
    private String login(String user, String password) {
        try {
            ...
            log.error("Openbank login failed", e);
        }
        return null;
    }

    private List<SyncResultMessage.ImportedTransaction> getTransactions(String token, String contract, String product, String from, Long accountId) {
        List<SyncResultMessage.ImportedTransaction> result = new ArrayList<>();
        try {
            String url = BASE_URL + "accounts/" + contract + "/" + product + "/transactions";
            ...
            for (JsonNode t : response.getBody().path("transactions")) {
                ...
                String emitter = t.path("emitter").asText("");
                String receiver = t.path("receiver").asText("");
                ...
                result.add(SyncResultMessage.ImportedTransaction.builder()
                        ...
                        .description(description).emitterName(emitter).receiverName(receiver)
                        .balance(balance).build());
            }
        } catch (Exception e) {
            log.error("Failed to fetch Openbank transactions", e);
        }
        return result;
    }
