...
La entidad `Recurrent` SHALL tener los campos: `name` (obligatorio), `periodicity` (días, nullable), `amount` (precio actual, decimal), `link` (URL, nullable), `establishment` (nombre del establecimiento, obligatorio).
...
- **THEN** el sistema devuelve 201 con `periodicity` null, `amount` 0.00 y `link` null
...
- **THEN** el sistema devuelve 200 con array de artículos, cada uno con `id`, `name`, `establishment`, `periodicity`, `amount`, `link`, `createdAt`, `updatedAt`

### Requirement: Actualización de artículo recurrente
El sistema SHALL permitir actualizar `name`, `establishment`, `periodicity` y `link` de un artículo recurrente. El campo `amount` NO SHALL ser actualizable directamente; solo se modifica vía historial de precios.
