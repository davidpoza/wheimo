...
export default (app) => {
  ...
  route.get('/:id?',
    ...
    async (req, res, next) => {
      const { id } = req.params
      const userId = req.user.id;
      const { accountId, tags, limit, sort, offset, from, to, search, min, max, operationType, isFav, isDraft, hasAttachments } = req.query;
      ...
      try {
        ...
        const transactions = await transactionService.findAll({
          accountId, userId, tags: tagsArray, from, to, min, max, limit, offset, sort, search, operationType, isDraft, isFav, hasAttachments
        });
        return res.status(200).json(transactions);
      } catch (err) {
        loggerInstance.error('🔥 error: %o', err);
